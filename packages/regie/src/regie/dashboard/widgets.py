...
class WelcomeDashboard(Vertical):
    ...
    def show_catalog(self, harnesses: Sequence[HarnessCatalogEntry]) -> None:
        """Adapt immutable public catalog rows to the presentation-only renderer."""
        self.update_harnesses(
            [
                {
                    "name": harness.name,
                    "icon": harness.icon,

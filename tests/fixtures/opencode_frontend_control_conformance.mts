...
assert.ok(
  pluginUrl,
  "usage: node opencode_frontend_control_conformance.mts <plugin file url>",
);
...
const api = {
  ...
  client: {
    session: {
      ...
      // The exact v2 SDK input the TUI's api.client accepts
      // (sdk.gen.ts promptAsync: flat parameters, sessionID -> path).
...
await waitFor(
  () =>
    host.frames.some(
      (frame) => frame.type === "hello" && frame.protocol === PROTOCOL,
    ),
  "hello",
);
...
request(
  ...
  "r1",
  "opencode.send",
  sendParams("op-1", "ses_A", "first prompt"),
);
await waitFor(
  () => responseFor(host.frames, "r1") !== undefined,
  "r1 accepted",
);
...
assert.ok(
  promptCalls[0]!.text === "first prompt" &&
    promptCalls[0]!.sessionID === "ses_A",
);
...
request(
  ...
  "r3",
  "opencode.send",
  sendParams("op-3", "ses_A", "busy prompt"),
);
...
request(
  ...
  "r4",
  "opencode.send",
  sendParams("op-4", "ses_B", "other prompt"),
);
await waitFor(
  () => responseFor(host.frames, "r4") !== undefined,
  "r4 wrong_session",
);
...
await waitFor(
  () => responseFor(host.frames, "r6") !== undefined,
  "r6 unsupported",
);
assert.equal(responseFor(host.frames, "r6")!.error?.code, "invalid_request");
request(host, "r7", "opencode.send", {
  operation_id: "op-7",
  native_session_id: "ses_A",
  prompt: "",
});
await waitFor(
  () => responseFor(host.frames, "r7") !== undefined,
  "r7 blank prompt",
);
...
request(
  ...
  "r8",
  "opencode.send",
  sendParams("op-8", "ses_A", "slow prompt"),
);
await sleep(50);
request(
  host,
...
await waitFor(
  () => responseFor(host.frames, "r9") !== undefined,
  "r9 in progress",
);
assert.equal(
  responseFor(host.frames, "r9")!.error?.code,
  "operation_in_progress",
);
pendingPrompt.resolve(undefined);
pendingPrompt = null;
await waitFor(
  () => responseFor(host.frames, "r8") !== undefined,
  "r8 accepted after slow send",
);
...
request(
  ...
  "r10",
  "opencode.send",
  sendParams("op-10", "ses_A", "lost prompt"),
);
await waitFor(
  () => responseFor(host.frames, "r10") !== undefined,
  "r10 transport",
);
...
await waitFor(
  () => responseFor(host.frames, "r11") !== undefined,
  "r11 cached unknown",
);
assert.equal(responseFor(host.frames, "r11")!.error?.code, "delivery_unknown");
assert.equal(promptCalls.length, 2);

// A server failure stays unknown: definite for the server, ambiguous for us.
api.client.session.promptAsync = async () => {
  return {
...
request(
  ...
  "r12",
  "opencode.send",
  sendParams("op-12", "ses_A", "missing session"),
);
await waitFor(
  () => responseFor(host.frames, "r12") !== undefined,
  "r12 server failure",
);
assert.equal(responseFor(host.frames, "r12")!.error?.code, "delivery_unknown");
assert.match(String(responseFor(host.frames, "r12")!.error?.message), /404/);
assert.equal(promptCalls.length, 2);
...
request(
  ...
  "r13",
  "opencode.send",
  sendParams("op-13", "ses_A", "doomed prompt"),
);
await sleep(50);
...
pendingPrompt = null;
await waitFor(
  () => responseFor(host.frames, "r13") !== undefined,
  "r13 unknown",
);
...
request(
  ...
  "r14",
  "opencode.send",
  sendParams("op-1", "ses_B", "replayed prompt"),
);
await waitFor(
  () => responseFor(host.frames, "r14") !== undefined,
  "r14 not cached",
);
...
await waitFor(
  () =>
    host.frames.some(
      (frame) =>
        frame.type === "event" && frame.event?.type === "message.updated",
    ),
  "lineage forwarded",
);
...
// Bounded recent receipts: sequential sends far past 64 all succeed; only
// the oldest settled receipt evicts, in-flight work never does.
for (let i = 0; i < 70; i += 1) {
  request(
    host,
    `seq-${i}`,
    "opencode.send",
    sendParams(`seq-op-${i}`, "ses_A", `prompt ${i}`),
  );
}
await waitFor(
  () =>
    host.frames.filter(
      (frame) =>
        frame.type === "response" && String(frame.id).startsWith("seq-"),
    ).length === 70,
  "sequential batch settled",
);
for (let i = 0; i < 70; i += 1) {
  assert.equal(
    responseFor(host.frames, `seq-${i}`)!.result?.status,
    "accepted",
    `seq-${i} accepted`,
  );
}
assert.equal(promptCalls.length, 4 + 70);
// A recent duplicate is still once-only: the cached terminal reply answers
// without another SDK call (Theater's durable store stops old replays).
request(
  host,
  "seq-dup",
  "opencode.send",
  sendParams("seq-op-69", "ses_A", "prompt 69"),
);
await waitFor(
  () => responseFor(host.frames, "seq-dup") !== undefined,
  "recent duplicate cached",
);
assert.deepEqual(
  responseFor(host.frames, "seq-dup")!.result,
  responseFor(host.frames, "seq-69")!.result,
);
assert.equal(promptCalls.length, 4 + 70);

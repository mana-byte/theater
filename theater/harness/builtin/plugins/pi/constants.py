"""Pi-native names and bounded adapter values."""

from __future__ import annotations

PI_BINARY = "pi"
PI_SESSIONS_DIRNAME = "sessions"
PI_ISOLATION_MARKER = ".theater-pi-source"
PI_SWITCH_MARKER = ".theater-pi-switch.json"
PI_SWITCHES_DIRNAME = ".theater-pi-switches"
PI_MARKER_KEY = "pi-domain-marker.key"
PI_MARKER_VERSION = 1
PI_SWITCH_MARKER_BYTES = 16 * 1024
PI_SWITCH_MARKER_VERSION = 1

# Keep one observer poll small even if an untrusted session file is damaged.
PI_READ_BYTES = 256 * 1024
PI_RECORD_BYTES = 8 * 1024 * 1024
PI_PROJECTED_RECORD_BYTES = 256 * 1024
PI_RECORD_TEXT_BYTES = 128 * 1024
PI_FIELD_TEXT_BYTES = 64 * 1024
PI_RECORDS_PER_BATCH = 128
PI_HEADER_BYTES = 64 * 1024

PI_LAUNCH_ENV: dict[str, str] = {
    "PI_OFFLINE": "1",
    "PI_CACHE_RETENTION": "long",
    "PI_SKIP_VERSION_CHECK": "1",
    "PI_TELEMETRY": "0",
}

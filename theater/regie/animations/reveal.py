"""Keyed reveal animation: controller, frame type, and pure clipping.

``LeafRevealController`` reveals the initial tree and eligible new leaves once
per stable key. ``clip_parts`` truncates styled text to a visible column count.
"""
...
#: A styled part is a plain string or a ``(text, style)`` pair.
...
class LeafRevealController:
    """Reveal the initial tree and eligible new leaves once per stable key."""
    ...
    def observe(
        ...
        *,
        animate_new: Collection[Key] = (),
        now: float | None = None,
    ) -> LeafRevealFrame:
        """Register unseen keys and animate only eligible additions after startup."""
        ...
        if not self._started:
            ...
            if self._enabled:
                eligible = set(animate_new)
                self._add_new([key for key in unseen if key in eligible], current)

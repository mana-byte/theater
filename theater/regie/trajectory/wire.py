"""Validated trajectory wire values and defensive decoding."""
...
def _decode_page_records(
    value: object, path: str, *, allow_uncursored_bound: bool = False
) -> tuple[tuple[TrajectoryRecord, ...], bool]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes, bytearray)):
        raise WireDecodeError(f"{path} must be an array")
    if len(value) > MAX_PAGE_RECORDS:
        raise WireDecodeError(f"{path} exceeds {MAX_PAGE_RECORDS} records")
    ...
    for index, item in enumerate(value):
        ...
        if next_size > MAX_RESPONSE_BYTES:
            if allow_uncursored_bound:
                truncated = True
                break
            raise WireDecodeError(f"{path} exceeds {MAX_RESPONSE_BYTES} estimated bytes")
...
class TrajectoryPage:
    ...
    def from_wire(
        ...
        decoded_participant_id = _identifier(participant_value, f"{path}.participant_id")
        allow_uncursored_bound = not any(
            raw.get(key) is not None for key in ("stream_id", "cursor", "older_cursor")
        )
        records, locally_truncated = _decode_page_records(
            raw.get("records", ()),
            f"{path}.records",
            allow_uncursored_bound=allow_uncursored_bound,
        )
        if any(record.participant_id != decoded_participant_id for record in records):
            raise WireDecodeError(f"{path}.records contain another participant")
        ...
        return cls(
            participant_id=decoded_participant_id,
            ...
            truncated_by_bytes=locally_truncated
...
class TrajectoryFollow:
    ...
    def from_wire(
        ...
        if any(record.participant_id != decoded_participant_id for record in upserts):
            raise WireDecodeError(f"{path}.upserts contain another participant")
        return cls(
            participant_id=decoded_participant_id,
            stream_id=_optional_string(
                raw.get("stream_id"), f"{path}.stream_id", limit=MAX_IDENTIFIER_BYTES

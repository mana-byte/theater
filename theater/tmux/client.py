...
# ---- key bindings --------------------------------------------------------
#
# `#{key}` silently expands to empty in list-keys -F output; `#{key_string}`
# is the field that actually holds the key name.
...
async def unbind_key_if_owned(table: str, key: str, *, note: str) -> None:
    ...
    so it does not distinguish between two concurrent callers racing on the
    same key — only between "ours" and "not ours".
    """
    out = await run(

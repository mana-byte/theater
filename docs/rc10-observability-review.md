...
| P2 | Reduce repeated process discovery | `lsof` averages 35.1 ms; `ps-comm` 14.3 ms. Reuse process snapshots and stop redundant discovery after exact identity is established, with explicit invalidation on process/session changes. Never substitute cwd/time correlation. |

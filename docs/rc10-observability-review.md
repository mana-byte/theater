...
| P2 | Avoid obsolete observer rebuilds | Fixed: `_start_watch` returns early for missing or dead participants, so a live-unregistration after an explicit kill no longer reopens observation. |
| P2 | Correct readiness timing on observer restarts | Fixed: birth-to-watch readiness records once per participant; every later rebuild is measured only as `observer.restart`. |

...
`transcript-receipt` command and its per-participant token. The retained `claude-receipt` command
serves settings generated by older Theater versions. Installed `claude --help` exposes
...
| Claude receipt migration | already implemented; retain compatibility alias | New settings use `transcript-receipt`; retain the legacy CLI/RPC aliases for existing settings. |
